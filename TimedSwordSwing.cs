...
public class TimedSwordSwing : MonoBehaviour
{     
    ...
    void Swinging() {
        ...
        if (isSwinging) {
            swingBufferTime = Mathf.Clamp(swingBufferTime + Time.deltaTime, 0f, 0.5f);
        }

        if (isSwinging && Input.GetMouseButtonUp(0)) {
            Debug.Log(swingBufferTime);
            swingStart = true;
        }

        else if (swingStart){
            swingTime += Time.deltaTime / swingBufferTime;
